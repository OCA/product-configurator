...
class ProductConfiguratorSale(models.TransientModel):
    ...
    def _get_order_line_vals(self, product_id):
        ...
        product = self.env["product.product"].browse(product_id).with_context(
    ...
    def action_config_done(self):
        ...
        if self.order_line_id:
            self.order_line_id.with_context(lang=self.order_id.partner_id.lang).write(
                values
            )
        else:
            self.order_id.with_context(lang=self.order_id.partner_id.lang).write(
