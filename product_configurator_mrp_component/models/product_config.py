...
class ProductConfigSession(models.Model):
    ...
    def create_get_bom(self, variant, product_tmpl_id=None, values=None):
        if product_tmpl_id:
            master_bom = self.env["mrp.bom"].search([("product_tmpl_id", "=", product_tmpl_id.id),("product_id", "=", False),],order="sequence asc",limit=1,)
            vals = False
            wizard_values = variant.product_template_attribute_value_ids.mapped("product_attribute_value_id")
            for config_component_line in master_bom.bom_line_config_ids:
                config_component_vals = config_component_line.product_tmpl_id.mapped("attribute_line_ids.value_ids")
                ...
                # Otherwise create config component variant and bom
                    ...
                    component_config_session = self.create_get_session(config_component_line.product_tmpl_id.id)
                    ...
                    bom_line_config_set = self.env["mrp.bom.line.configuration.set"].search([("name", "=", component_variant.display_name)],limit=1,)
                    if not bom_line_config_set:
                        bom_line_config_set = self.env["mrp.bom.line.configuration.set"].create({"name": component_variant.display_name,})
                    ...
                    existing_bom_line = self.env["mrp.bom.line"].search([("bom_id", "=", master_bom.id),("product_id", "=", component_config_session.product_id.id),("config_set_id", "=", bom_line_config_set.id),],limit=1,)
